...
export default function CommunityMessage({
  ...
  if (!isPoll && hasImageAttachment) {
    ...
    // Convert MessageAttachment to Attachment format for SocialPost
    const convertedAttachments = nonImageAttachments.map(att => ({
      type: att.type,
      url: att.url,
      name: att.name || undefined,
      size: att.size ? parseInt(att.size, 10) || undefined : undefined
    }));
    
    // Make sure the image URL is properly formatted
    let imageUrl = mainImage?.url;
    
    // Check if the URL exists and ensure it's properly formatted
    if (imageUrl) {
      ...
      // If it's an absolute URL but missing the origin, add it
      if (imageUrl.startsWith('/') && typeof window !== 'undefined') {
        // Use window.location.origin only on the client side
        // The SocialPost component will further process this
